...
FormValidator.prototype = {
    ...
    tests : {
        sameAsPlaceholder : function( $field, data ){
            if( $field.prop('placeholder') )
                return data.value != $field.prop('placeholder') || this.getErrorText($field, "empty");
            else
                return true;
        },
        ...
        linked : function(a, b, type){
            if( b != a ){
                ...
                return getErrorText(type + '_repeat') || message.no_match;
            }
            return true;
        },
        ...
        email : function($field, data){
            if ( !this.settings.regex.email.filter.test( data.value ) || data.value.match( this.settings.regex.email.illegalChars ) ){
                return this.getErrorText($field, "email");
            }
        ...
        text : function($field, data){
                ...
                var wordsLength = function(len){
                    for( var w = words.length; w--; )
                        if( words[w].length < len )
                            return this.getErrorText($field, "short");
                    return true;
                };

                if( words.length < data.validateWords || !wordsLength(2) )
                    return this.getErrorText($field, "complete");
            ...
            if( data.lengthRange && data.lengthRange[1] && data.value.length > data.lengthRange[1] ){
                return this.getErrorText($field, "long");
            }
            ...
            if( data.pattern ){
                ...
                try{
                    ...
                    if( data.value && !jsRegex ){
                        return this.getErrorText($field, "invalid");
                    }
                }
                catch(err){
        ...
        number : function( $field, data ){
            ...
            if( isNaN(parseFloat(a)) && !isFinite(a) ){
                return this.getErrorText($field, "number");
            }
            // not enough numbers
            ...
            // check if there is max length & field length is greater than the allowed
                ...
                return this.getErrorText($field, "number_min");
            }
            else if( data.minmax[1] && (a|0) > data.minmax[1] ){
                return this.getErrorText($field, "number_max");
            }
        ...
        date : function( $field, data ){
            ...
            for( i = A.length; i--; ){
                if( isNaN(parseFloat( data.value )) && !isFinite(data.value) )
                    return this.getErrorText($field, "date");
            }
            try{
        ...
        url : function( $field, data ){
            ...
            if( !this.settings.regex.url.test(data.value) )
                return this.getErrorText($field, "url");
        ...
        select : function( $field, data ){
            return data.value ? true : this.getErrorText($field, "select");
        },

        checkbox : function( $field, data ){
            if( $field[0].checked ) return true;

            return this.getErrorText($field, "checked");
        }
    },
    ...
    */
    ...
    getErrorText: function($field, text)
    {
        var customErrorAttr = $field.attr('data-error-msg'); 

        if(typeof customErrorAttr !== typeof undefined && customErrorAttr !== false)
        {
            return customErrorAttr;
        }
